...
import { Activity, Users, Zap, Globe, ArrowRight, RefreshCw, Clock, ExternalLink } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const fetchLogs = async () => {
    try {
      setLogsLoading(true)
      setLogsError(null)
      const response = await fetch("/api/explorer/logs?limit=10")
      const data = await response.json()
      if (data.success) {
        setLogs(data.logs || [])
      } else {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-white/50 max-w-none font-light leading-relaxed">
            Real-time network statistics from the Avalanche Fuji facilitator network
          </p>
        </motion.div>
        ...
        {/* ── Section 3: Facilitator Leaderboard ──────────────────────── */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
            ...
            <div className="space-y-2">
              ...
              {facilitators.map((facilitator, index) => (
                <React.Fragment key={facilitator.id}>
                  ...
                  >
                    <div className="col-span-1 font-mono text-sm text-white/60">
                      #{index + 1}
                    </div>
                    <div className="col-span-3 font-mono text-sm text-white/90 truncate">
                      {facilitator.name}
                    </div>
                    <div className="col-span-3 font-mono text-xs text-white/40">
                      {truncateAddress(facilitator.facilitatorWallet)}
                    </div>
                    <div className="col-span-2">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-mono font-bold uppercase border ${
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${
                            facilitator.status === "active" ? "bg-green-500" : "bg-white/20"
                          }`}
                        />
                        {facilitator.status}
                      </span>
                    </div>
                    <div className="col-span-2 font-mono text-sm text-white/70">
              ...
              {logs.map((log, index) => (
                ...
                >
                  ...
                  <div className="md:w-36 shrink-0">
                    <span
                      className={`inline-flex px-2 py-1 rounded text-[10px] font-mono font-bold uppercase border ${getEventBadgeStyle(log.eventType)}`}
                    >
        ...
        {/* ── Section 5: Network Info ──────────────────────────────────── */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.8, duration: 0.5 }}
          className="rounded-xl border border-white/10 bg-black/40 backdrop-blur-xl p-6 md:p-8"
        >
